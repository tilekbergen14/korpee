  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        { name, price, spending, id: selectedIndex },
        {
          headers: {
              ...
              {items.length > 0 ? (
                items.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell>{item.spending}м</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
...
export const getStaticProps = async () => {
  try {
    const items = await axios.get(`${process.env.server}/service`);
    return {
