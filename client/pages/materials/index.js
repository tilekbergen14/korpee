  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.server}/material`,
        { name, price, total, id: selectedIndex },
        {
          headers: {
      ...
      if (selectedIndex) {
        setItems(items.map(item => (item._id === selectedIndex ? { ...item, name, price, total } : item)));
      } else {
...
export const getStaticProps = async () => {
  try {
    const items = await axios.get(`${process.env.server}/material`);
    return {
